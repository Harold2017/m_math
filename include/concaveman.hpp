//
// Created by Harold on 2021/6/2.
//

#ifndef M_MATH_CONCAVEMAN_HPP
#define M_MATH_CONCAVEMAN_HPP

// code from: https://github.com/sadaszewski/concaveman-cpp
//
...
// segment to segment distance, ported from http://geomalgorithms.com/a07-_distance.html by Dan Sunday
            ...
            public:
                ...
                void insert(data_type data, const bounds_type &bounds) {
                    ...
                    if (m_children.size() < MAX_CHILDREN) {
                        auto r = details::make_unique<type>(data, bounds);
                    ...
                    auto leaf = details::make_unique<type>(best_child.get().data(),
                ...
                void print(int level = 0) {
                    ...
                    for (auto it = m_children.begin(); it != m_children.end();) {
                        ...
                        if ((*it)->m_is_leaf) {
                            ...
                            printf("%s branch %0.6f %0.6f %0.6f %0.6f \n", pad.c_str(), bounds[0], bounds[1], bounds[2],
                                   bounds[3]);
                            (*it)->print(level + 1);
                        }
                        it++;
            ...
            struct Node {
                ...
                explicit Node(const point_type &p) : Node() {
                    this->p = p;
                }
            ...
            public:
                ...
                template<class... Args>
                explicit CircularElement<T>(Args &&... args):
...
// check if the edge (a,b) doesn't intersect any other edges
        ...
        std::vector<std::array<T, 2>> concaveman(
            ...
            typedef details::Node<T> node_type;
            typedef std::array<T, 2> point_type;
            ...
            for (auto &elem : queue) {
                auto &node(elem->data());
                details::updateBBox<node_type>(elem);
...
snapshot(a, b, c, d, sqLen, maxSqLen, trigger, true);
                ...
                if (ok && std::min(details::getSqDist(p, b), details::getSqDist(p, c)) <= maxSqLen) {
                    ...
                    details::updateBBox<node_type>(elem);
                    details::updateBBox<node_type>(elem->next());
