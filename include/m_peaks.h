...
namespace M_MATH {
    ...
    inline std::pair<int,int> counts_vp(T const& array, size_t row, size_t col, GetterFp fp) {
        int cnt_v = 0 ;
        int cnt_p = 0 ;
...
// not consider plateau
    template <typename T, typename GetterFp>
    inline auto find_peaks_and_valleys(T const& array, size_t rows, size_t cols, GetterFp fp)
...
// not consider plateau
    template <typename T, typename GetterFp>
    inline auto find_peaks(T const& array, size_t rows, size_t cols, GetterFp fp)
...
// not consider plateau
    template <typename T, typename GetterFp>
    inline auto find_valleys(T const& array, size_t rows, size_t cols, GetterFp fp)
    -> std::vector<std::pair<size_t, size_t>> {
