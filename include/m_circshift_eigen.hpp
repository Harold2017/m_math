...
#ifndef M_MATH_M_CIRCSHIFT_EIGEN_HPP
#define M_MATH_M_CIRCSHIFT_EIGEN_HPP

// code from https://stackoverflow.com/questions/46077242/eigen-modifyable-custom-expression/46301503#46301503
// modified by Harold to meet Cpp11
...
namespace helper
{
    ...
    template <typename T>
    struct is_matrix : decltype(detail::is_matrix(std::declval<typename std::remove_cv<T>::type>()))
    {
    };

    template <typename T>
    struct is_array : decltype(detail::is_array(std::declval<typename std::remove_cv<T>::type>()))
    {
    };
 ...
 */
    ...
    auto index_if_necessary(T&& thing, Eigen::Index idx) -> decltype(thing)
    {
        return thing(idx);
    }
 ...
 * Overload for scalar.
 */
    template <typename T, typename std::enable_if<std::is_scalar<typename std::decay<T>::type>::value, int>::type = 0>
    auto index_if_necessary(T&& thing, Eigen::Index) -> decltype(thing)
    {
        return thing;
    }
}
...
#endif //M_MATH_M_CIRCSHIFT_EIGEN_HPP
